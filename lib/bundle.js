...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.prototype.newQuestions = function () {
	  for(var i = 0; i < 3; i++){
	    this.questions.push(new Question(this.blockCount));
	  }
	...
	Game.prototype.handleClick = function (event) {
	  ...
	  this.blocks.forEach(function(block){
	    if(event.offsetX >= block.pos[0] && event.offsetX <= block.pos[0]+block.width){
	      if(event.offsetY >= block.pos[1] && event.offsetY <= block.pos[1] + block.height){
	        clickedBlock = true;
	        block.highlight();
...
/***/ function(module, exports) {
	...
	var Question = function(blockCount){
