...
/***/ function(module, exports) {
	...
	GameView.prototype.animate = function(time){
	  ...
	  if(Math.floor(time / 1000) % 120 === 0 && time > 1000){
	    console.log("winner");
	    this.won = true;
	  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	Game.prototype.handleClick = function (event) {
	  var game = this;
	  this.blocks.forEach(function(block){
	    if(event.x >= block.pos[0] && event.x <= block.pos[0]+block.width){
	      if(event.y >= block.pos[1] && event.y <= block.pos[1] + block.height){
	        block.highlight();
...
/***/ function(module, exports) {
	...
	var Block = function(num, idx){
	  ...
	  this.pos = [Math.floor(Math.random() * 8) * 125,100];
	  this.maxHeight = 600;
	  this.num = num;
